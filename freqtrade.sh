...
[ -z "${TRADINGMODE:-}" ] && echo "ERROR: 'TRADINGMODE' env var not defined" && exit 1
...
docker run \
	...
	-c /configs/common.json \
	-c /configs/exchange-${EXCHANGE}.json \
	-c /configs/stake-${STAKE}.json \
	-c /configs/tradingmode-${TRADINGMODE}.json
