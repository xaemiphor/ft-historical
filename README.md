...
```
      - name: Checkout Exchange Data
        ...
        with:
          ...
          ref: "${{ matrix.tradingmode || 'spot' }}/${{ matrix.exchange || 'binanceus' }}/${{ matrix.stake | 'USDT' }}/${{ matrix.timerange || '202412' }}"
          path: user_data/data/${{ matrix.exchange || 'binanceus' }}
```

Additionally, can be used locally
```
git clone --depth 1 --branch ${TRADINGMODE:-spot}/${EXCHANGE:-binanceus}/${STAKE:-USDT}/${TIMERANGE:-202412} https://github.com/xaemiphor/ft-historical.git user_data/data/${EXCHANGE:-binanceus}-${TIMERANGE:-202412}
```
